...
import { UsersData } from '@core/models/user.interface';
import { env } from '@env';
...
// donc "abc" (string' devient true (boolean)) */
  ...
  async login(identifier: string, password: string): Promise<void> {
    ...
    const response = await firstValueFrom(
      this._httpClient.post<LoginReponse>(this._apiUrl + 'auth/user/login', body),
    );
  ...
  async register(userData: UsersData): Promise<void> {
    await firstValueFrom(
      ...
      this._httpClient.post(this._apiUrl + 'auth/user/register', userData),
    );
  }
  logout() {
    this._authToken.set('');
